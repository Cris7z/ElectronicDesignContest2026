...
/* Must match app/main.c's live C07A configuration for D-transition tests. */
static const line_tracker_config_t k_live_d_config = {
    ...
    .centroid_min_strength = 150U,
    .max_track_black_count = 3U,
    .lost_limit_ticks = 60U,
    .lost_confirm_ticks = 3U,
    .cross_confirm_ticks = 2U,
    .center_gap_limit_ticks = 0U,
    .center_gap_error_limit = 0.20f,
    .speed_start_duty = 0.0800f,
    .speed_max_duty = 0.4875f,
    .speed_min_duty = 0.2300f,
    .speed_filter_alpha = 0.20f,
    .speed_deadband_weight = 1.0f,
    .speed_full_slow_weight = 4.0f,
    ...
    .error_filter_alpha = 0.60f,
    .pid_d_filter_alpha = 0.35f,
    .pid_d_limit_duty = 0.100f,
    .yaw_limit_duty = 0.450f,
    .edge_yaw_duty = 0.300f,
    .yaw_gain_min = 0.90f,
    .yaw_gain_start_weight = 1.0f,
    .yaw_gain_full_weight = 4.0f,
    .edge_blend_start_weight = 5.0f,
...
static line_tracker_input_t input_with_center_line(void)
{
    line_tracker_input_t input = input_with_black(3U);

    input.raw_adc[4] = 4095U;
    return input;
}
...
static void test_live_d_limit_bounds_entry_and_exit_kicks(void)
{
    line_tracker_t tracker;
    line_tracker_input_t centre = input_with_center_line();
    line_tracker_input_t weight_one = input_with_black(3U);
    unsigned tick;

    assert(line_tracker_init(&tracker, &k_live_d_config));
    centre.start_event = true;
    line_tracker_step(&tracker, &centre);
    centre.start_event = false;
    line_tracker_step(&tracker, &centre); /* Seed zero-error D history. */

    line_tracker_step(&tracker, &weight_one);
    assert(tracker.output.pid_d_limited);
    assert(fabsf(tracker.output.pid_d_duty - 0.100f) < 0.0001f);
    assert(fabsf(tracker.output.yaw_final_duty) <= 0.1300f);
    ...
    assert(fabsf(tracker.output.pid_d_duty) < 0.0001f);

    line_tracker_step(&tracker, &centre);
    assert(tracker.output.pid_d_limited);
    assert(tracker.output.yaw_final_duty < 0.0f);
    assert(fabsf(tracker.output.yaw_final_duty) <= 0.0800f);
}

static void test_live_d_limit_keeps_large_transition_within_yaw_limit(void)
{
    line_tracker_t tracker;
    line_tracker_input_t centre = input_with_center_line();
    ...
    for (tick = 0U; tick < 8U; ++tick) {
        ...
        assert(fabsf(tracker.output.pid_d_duty) <=
               k_live_d_config.pid_d_limit_duty + 0.0001f);
        assert(fabsf(tracker.output.yaw_final_duty) <=
               k_live_d_config.yaw_limit_duty + 0.0001f);
    }
}

static void test_live_d_limit_preserves_start_and_reacquisition_seeding(void)
{
    line_tracker_t tracker;
    line_tracker_input_t weight_one = input_with_black(3U);
    line_tracker_input_t missing = input_with_black(8U);
...
int main(void)
{
    ...
    test_live_d_limit_bounds_entry_and_exit_kicks();
    test_live_d_limit_keeps_large_transition_within_yaw_limit();
    test_live_d_limit_preserves_start_and_reacquisition_seeding();
