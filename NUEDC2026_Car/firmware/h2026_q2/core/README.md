...
- Pass the raw HiWonder register-5 byte and set `line_i2c_valid=false` for
  every failed or stale read. Invalid bytes never enter marker or PD logic.
...
inject measured sensor polarity/order, left and right metres per encoder
count, encoder signs, track width, duty/feed-forward values, speed-loop
