...
> C07A 模块化控制器、D157B、YB-MVX05 和闭环步进组件不匹配，
> **不得直接烧录上车**。

## 1. 赛题约束导出的设计原则

- 车体外廓不超过 35 cm × 25 cm，车轮驱动，电池随车。
- 循迹只能使用红外光电模块；主传感器为 YB-MVX05-V1.0 八路复用灰度，
  K230 不参与赛道线识别。
...
```mermaid
flowchart LR
    GS["YB-MVX05-V1.0\n八路复用灰度\nLF04 同口备份（断电换装）"] --> MCU["TI MSPM0G3507 模块化控制器\nC07A + S27F/S28A\n唯一实时控制主机"]
    WE["左右轮编码器"] --> MCU
...
| 节点 | 只负责什么 | 明确不负责什么 |
|---|---|---|
| MSPM0G3507 | 八路灰度扫描、轮速、姿态采样、任务状态机、球位置外环、杆角内环、失效保护 | 图像处理、视频传输 |
...
| YB-MVX05-V1.0 八路灰度 | 已有资料；5 V 供电，以 `AD0～AD2` 选通 CD4051，`OUT` 返回一路数字状态 | 主循迹传感器，软件轮询八路 |
| LF04 四路独立红外模块 | 已购；O1～O4 四路数字量，与 YB-MVX05 共用同一组四个 GPIO | 同口备用；须断电物理换装，只允许与 YB-MVX05 二选一 |
...
| 本地 WHEELTEC/YB-MVX05 例程 | 与实物接口一致，适合逐模块点亮 | 只移植底层驱动，不照抄旧整车引脚 |
...
- YB-MVX05 位于车头下方、横向居中，可调高度与俯仰；先按资料建议以探头
  离地约 18 mm 为装调起点，再用实际赛道标定八路有效极性、左右顺序和黑白
  电平裕量。换装 LF04 备份时重新按其结构调整高度，不沿用八路模块标定值。
...
以下分配采用厂商 C07A 例程的 D157B 和左右轮编码器引脚，把 YB-MVX05 适配到
S27F/S28A 的红外接口，并针对已确认的 **D36A** 解决原例程冲突。该表是首版
冻结目标，但首次上电前仍要按实物丝印逐线通断检查。首版保留 MPU6050，
物理拔下 D103A/TB6612 和蓝牙模块。
...
| YB-MVX05 OUT | PB17 | 数字输入；读取当前选通通道 |
| YB-MVX05 AD0/AD1/AD2 | PB16 / PA12 / PA27 | 推挽输出；首版选通后等待约 100 µs 再采样 |
| YB-MVX05 VCC/GND | 另取 5 V / GND | U3 Pin 2 是 3.3 V，**不得**作为模块 5 V；自制线束 |
| MPU6050 SCL/SDA/INT | PA1 / PA0 / PA7 | 保留 H5 插接模块；I2C0 + 中断，先核对模块供电和总线上拉电压 |
...
YB-MVX05 不是八路并行模拟量，也不是 UART/I2C/SPI：固件输出三位地址，
等待模拟开关和比较器稳定，再从 `OUT` 取得一个数字状态，循环扫描八路。通道
有效极性和 X1～X8 左右顺序必须逐探头遮挡标定。LF04 备份沿用同一组 GPIO：
`O1/O2/O3/O4 = PA27/PA12/PB16/PB17`；换装时加载另一套驱动和标定参数，
两块循迹板不得同时连接。YB-MVX05 资料未给出 `OUT` 高电平的精确电压，首上电
先在不接 PB17 时测量；只有确认与 MSPM0 输入兼容后才直连，否则加电平转换。
...
YB-MVX05 的 AD1、MPU6050 和左右轮编码器；这里只保留 STEP、DIR。EN 改为
...
- P03B 插在 H15/H16，负责模块化控制器的 5 V 逻辑；其本地资料没有给出可信
  额定电流，不能凭商品标题假定 3 A 或 5 A。先分别测 C07A/OLED/MPU/灰度和
...
| YB-MVX05 循迹 PD | 100～200 Hz | 八路复用黑白状态 | 左右轮差速目标 |
...
2. **供电门**：分别测 P03B 逻辑负载、轮堵转、步进锁止和 K230 启动/推流
   ...
   无异常发热后才能冻结。
3. **底盘门**：架空开环确认左右电机/编码器符号，低速闭环直行，再上
   YB-MVX05；逐通道确认地址、有效电平和左右顺序。LF04 只做同口替换验收。
