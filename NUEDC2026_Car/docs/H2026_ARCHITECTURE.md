...
> C07A 模块化控制器、当前 TB6612、HiWonder 八路红外和闭环步进组件不匹配，
> **不得直接烧录上车**。

> **2026-07-30 当前实物覆盖说明：** 底盘实际使用 H9/H12 上的
> D103A/TB6612，不使用外接 D157B。架空实测确认 A 路
> `PB2 + PA14/PA13` 是物理右轮，B 路 `PB3 + PA16/PA17` 是物理左轮；两路最初
> 正电气命令均为后退，固件已交换逻辑左右并反转正方向，待下一轮架空复验。因此本文其余所有“D157B”
> 作为当前主驱动、或“拔下 TB6612 以启用 D36A”的描述均为历史方案；D36A 和
...
```mermaid
flowchart LR
    GS["HiWonder LineFollower_8CH v1.0\n八路红外 / UART 115200"] --> MCU["TI MSPM0G3507 模块化控制器\nC07A + S27F/S28A\n唯一实时控制主机"]
    LF["LF04 四路红外\n规则备用 / U3"] -.-> MCU
    WE["左右轮编码器"] --> MCU
...
以下分配采用厂商 C07A TB6612 和左右轮编码器引脚。当前实物试验把
HiWonder 原生 UART 放到 H8 的 PB6/PB7，并针对已确认的 **D36A** 解决原例程
冲突。此前 H8 通断结果异常，因此这一项以实际 UART 应答和逐路遮挡验证为准。
当前保留 MPU6050 和 D103A/TB6612，物理拔下蓝牙模块。

### 5.1 底盘与板载资源

| 功能 | MSPM0 引脚 | 电气/备注 |
|---|---|---|
| 左电机 B | PB3 + PA16/PA17 | TIMA1_CCP1=PWMB；BIN1/BIN2 定方向 |
| 右电机 A | PB2 + PA14/PA13 | TIMA1_CCP0=PWMA；AIN1/AIN2 定方向 |
| 左轮编码器 A/B | PA25 / PA26 | GPIO 中断软件正交计数 |
| 右轮编码器 A/B | PB20 / PB24 | GPIO 中断软件正交计数 |
| HiWonder 8CH UART | PB6 TX / PB7 RX，UART1 | H8 Pin 2/3；交叉接传感器 RX/TX，115200 8N1 |
| HiWonder 8CH 5V/GND | H8 Pin 5 / Pin 4 | 约 85 mA；蓝牙模块必须物理拔下 |
| MPU6050 SCL/SDA/INT | PA1 / PA0 / PA7 | 保留 H5 原装模块；PA7 只给 MPU6050，中断不接八路传感器 |
| LF04 O1/O2/O3/O4 | PA27 / PA12 / PB16 / PB17 | 厂商线序对应 U3 Pin 6/5/4/3；Q2 主传感器占用 Pin 4/3，LF04 不得同时插入 |
...
HiWonder 当前不用 I²C，也不与 MPU6050 共用 H5。H5 保留 `PA1/PA0/PA7` 给
原装 MPU6050；HiWonder 通过 H8 的 `5=5V、4=GND、2=PB6/TX→RX、
3=PB7/RX←TX` 使用 UART1 115200 8N1。固件先发 `0` 进入手动模式，再周期性
发 `1` 接收一字节 S1..S8 状态。U3 SDA/SCL 与传感器断开，LF04 和 K230 也不能
同时接入 PB6/PB7。此前 H8 通断测试曾异常，故只有 UART 响应计数增加且八路
遮挡能改变对应 bit 才算验证通过。
...
- D103A/TB6612：D157B 已作为主轮驱动，且它与 D157B、D36A 占脚冲突；
  从 H9/H12 物理拔下。
- 插接式蓝牙：从模块座物理拔下；Q2 八路红外改用 U3 PB16/PB17。
...
| HiWonder 8CH 循迹 PD | 100～200 Hz | UART 命令 1 返回的八路目标状态 | 左右轮差速目标 |
