...
> 版本：v0.3，2026-07-29
> 状态：D36A 型号已确认；最终视觉板冻结为拟购的 01Studio CanMV K230
> 标准版（建议 1 GB，标配 GC2093）。接口已到“可分模块开工”，电平、有效极性和峰值电流
> 仍须在实物上逐线验收。
> 重要：当前仓库原有 `bsp/car_mspm0g3507.syscfg` 是旧通用小车模板，与已购
> S28A/C07A、D157B、LF04 和闭环步进组件不匹配，**不得直接烧录上车**。

## 1. 赛题约束导出的设计原则

- 车体外廓不超过 35 cm × 25 cm，车轮驱动，电池随车。
- 循迹只能使用红外光电模块，K230 不参与赛道线识别。
...
因此首要原则是：**控制数据面与视频传输面逻辑隔离**。二者可以共用 K230
和 GC2093，但 MSPM0 只接收带时戳、序号和 CRC 的球状态帧；RTSP 卡顿、录像
或接收端故障不得进入控制闭环。
...
| 节点 | 只负责什么 | 明确不负责什么 |
|---|---|---|
| MSPM0G3507 | 循迹、轮速、任务状态机、球位置外环、杆角内环、失效保护 | 图像处理、视频传输 |
| 01Studio CanMV K230 标准版 + 板载 GC2093 | 从同一相机管线测量 `x/vx/置信度` 并经 UART 发最新状态；同时输出 RTSP | 直接驱动任何执行器、赛道循迹 |
| 便携路由器 + 场外笔记本 | 建立专用 2.4 GHz 局域网；显示、存储、回放 RTSP | 参与控制或给 MSPM0 下发实时指令 |
...
| 硬件/资料 | 当前结论 | 首版用途 |
|---|---|---|
| WHEELTEC C07A（MSPM0G3507）+ S28A 底板、OLED、蓝牙 | 已购；底板版本待到货看丝印 | 唯一主控；板载 OLED 作合规计时屏 |
| D157B 双 AT8236 驱动板 | 已购；厂商例程为四路双输入 PWM | 左右 12 V 电机 |
| 32 cm × 24 cm 三轮差速底盘、MG513XP28_12V | 已购；编码器线数、减速比、堵转电流待实测 | 小车底盘 |
| LF04 四路独立红外模块 | 已购；是 O1～O4 四路数字量，不是“4 位编码总线” | 唯一循迹传感器 |
| 42 步进 + 编码器 + D36A + 球杆传动件 | 已确认 D36A；细分、EN 极性和电流仍须按实物登记 | 球杆闭环执行器 |
| 正点原子 K230D BOX + GC2093 | 已购；无无线图传能力 | 仅作算法台架和备用，不作为最终车载板 |
...
| MPU6050 | 已购但 PA0/PA1 与步进编码器 QEI 冲突 | 首版不接；后续有明确收益再迁移 |
| 25 cm PPR 管、钢球、铰链、相机支架、椭圆赛道 | 已购/已有 | 机械样机和测试场 |
| 本地 WHEELTEC/正点原子例程 | 与实物接口一致，适合逐模块点亮 | 只移植底层驱动，不照抄旧整车引脚 |
...
1. 图示 01Studio CanMV K230 标准版套装和专用 2.4 GHz 便携路由器/接收录像链路；
2. 合适的动力电池、保险/总开关、5 V 独立降压和电源分配；
3. 电机编码器 CPR、减速比、左右相序和堵转电流；
4. S28A 具体板本、D36A 细分/EN 有效极性，以及“视觉 + UART + RTSP”
   并行压力测试结果；
5. 所有模块装车后的实际外廓和重心。
...
- 标准版标配 70° GC2093，24P、默认接 CSI2。若把 70° 暂按水平视场粗算，
  ...
  GC2093。现有 K230D 摄像头不得在 FPC 针数和引脚定义未核对前直接互插。
  官方摄像头、延长线和 CSI 说明：
...
- LF04 位于车头下方、横向居中，可调高度与俯仰；先以离地约 8～12 mm 为
  装调起点，最终以黑白电平裕量实测为准。
...
以下分配采用厂商 C07A 例程的 D157B、编码器、LF04 通信引脚，并针对已确认的
**D36A** 解决了原例程冲突。该表是首版冻结目标，但首次上电前仍要按实物
丝印逐线通断检查。它有意停用板载蓝牙和 MPU6050。
...
| LF04 O1/O2/O3/O4 | PA27 / PA12 / PB16 / PB17 | 3.3 V 数字输入；有效极性台架标定 |
...
| Encoder A | PA1 / TIMG8_CCP0 | MS42CG A | 唯一硬件 QEI 给球杆 |
...
厂商 D36A 闭环例程原映射 `PA24/PA13/PA12 + PA1/PA0/PB20/PA25` 会撞 LF04
O2 和左右轮编码器；只有 STEP、DIR、A/B 得以保留。新 PWM 捕获脚 PA14 和
EN 脚 PA22 必须在到货后用独立例程验收，正式 SysConfig 不得自动分配。
S28A 上 PA14 同时连到 H12 的 TB6612 控制口，PA22 同时引到 CCD/J3-CN2；
...
| 3.3 V / 5 V | **均不接主控电源** | — | 两板独立供电，只接 TX 和 GND |

PB6/PB7 与 C07A 板载蓝牙共用；接 K230 前必须物理移除/断开蓝牙模块 TX，
仅在软件中禁用蓝牙不够，避免总线争用。4P 座按板端丝印逐一核对
...
- MPU6050：PA0/PA1 已给步进 A/B 编码器。
- 板载蓝牙：PB7 给 K230，模块必须断开。
...
flowchart TD
    ...
    PD --> B5K["独立低噪声 5 V / >=2 A 降压"]
    B5K --> K["40Pin 红色 5V + 黑色 GND\n锁止线束 → CanMV K230"]
    D157 --> C07["D157B 5 V：C07A、OLED、LF04"]
```

- 动力支路和逻辑支路只在星形分配点汇接，电机/步进回流不穿过 K230 地线。
- D157B、D36A 和降压模块入口分别放近端电解与 0.1 µF 陶瓷去耦。
- K230 标准版官方额定供电为 5 V @ 1 A；车上使用独立 5 V、持续能力不低于
  2 A 的降压模块留启动和编码余量，不能由 C07A/D157B 的 5 V 口反向供电。
- 台架首次启动可用套装 Type-C 线；正式装车冻结为独立降压经带锁止和应力
  释放的线束接标准板 40Pin 中红色 5 V、黑色 GND。严禁误接 3.3 V，也不得
  从 UART 4P 座的 3V3 给主板供电。官方供电说明：
  <https://wiki.01studio.cc/docs/canmv_k230/getting_start/power_supply/>。
- 电池容量、C 倍率和保险值不能凭商品标题决定；先测两轮堵转、步进锁止和
  K230 同时视觉+推流时的峰值电流，再按实测峰值留 30% 以上余量。
...
板载 GC2093 → 单一 Sensor(id=2)
       ...
       → 2.4 GHz Wi-Fi STA → 自带便携路由器
       → 场外笔记本 VLC/OBS → 实时显示 + 录像 + 立即回放
```

- 不依赖赛场公共 Wi-Fi；自带只服务 K230 和笔记本的 2.4 GHz 路由器。
...
| LF04 循迹 PD | 100～200 Hz | 四路黑白状态 | 左右轮差速目标 |
...
2. **供电门**：分别测静态、轮堵转、步进锁止、K230 启动峰值；无复位、无
   5 V 跌落、无异常发热。
3. **底盘门**：架空开环确认左右电机/编码器符号，低速闭环直行，再上 LF04。
4. **步进门**：D36A 先核对额定电源、3.3 V 编码器、细分、限流和 EN 安全态；
   先做 ±2°，再做 ±6°，累计角与绝对 PWM 不得漂移。
...
1. 图示 01Studio CanMV K230 标准版套装和专用 2.4 GHz 路由器尚未上台架，
   因此无线图传目前只有官方资料依据，没有本机实测结论。
2. 当前底盘横向余量只有 1 cm，装车后极易超宽。
3. 现有旧 SysConfig 和 BSP 引脚是通用模板，不是 S28A 实物配置；需等板本、
   电机 CPR/减速比/堵转电流核实后再生成正式 `syscfg`。
4. D36A 型号已确定，但实物细分、限流档、EN 有效电平和失能安全态尚未台架
   验证；在完成低压逻辑检查前不得接动力电。
5. 标配 GC2093 的实际视场、钢球像素直径和“视觉 + UART + RTSP + OBS”
   2 小时并行稳定性尚未验证。
