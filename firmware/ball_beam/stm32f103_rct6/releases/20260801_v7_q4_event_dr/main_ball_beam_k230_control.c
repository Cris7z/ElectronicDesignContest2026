 ...
 * Board key K3 -> PC8, active low with the MCU internal pull-up enabled.
...
#define CONTROL_FIRMWARE_VERSION       "BALL_BEAM_F103_V7_Q4_EVENT_DR_PA10_RCT6"
...
static void VideoModeButton_Service(void)
{
    ...
    raw_released = (GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_8) != Bit_RESET) ? 1U : 0U;
    if(raw_released != s_k3_raw_released)
    {
        s_k3_raw_released = raw_released;
        s_k3_debounce_ticks = 0U;
    }
    else if(s_k3_debounce_ticks < K3_DEBOUNCE_TICKS)
    {
        s_k3_debounce_ticks++;
        if((s_k3_debounce_ticks >= K3_DEBOUNCE_TICKS) &&
           (s_k3_stable_released != s_k3_raw_released))
        {
            s_k3_stable_released = s_k3_raw_released;
...
static void PrintStatus(void)
{
    ...
    s_last_log_ms = g_ms;
    printf("BALL,t=%lu,state=%u,raw_mm=%ld,filt_x10=%ld,conf=%u,vel_x10=%ld,target=%ld,count=%ld,rpm_x100=%ld,brake_x10=%ld,age=%lu,valid=%lu,dup=%lu,invalid=%lu,parse=%lu,events=%lu,peak_mm=%ld,peak_vel_x10=%ld,peak_inner=%ld\r\n",
...
int main(void)
{
    ...
    printf("VIDEO_MODE_DEFAULT,0,K3_PC8_TO_K230_GPIO4_VIA_PA9\r\n");
