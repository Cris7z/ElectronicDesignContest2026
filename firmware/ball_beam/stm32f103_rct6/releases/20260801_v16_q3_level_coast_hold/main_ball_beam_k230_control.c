...
#define CONTROL_FIRMWARE_VERSION       "BALL_BEAM_F103_V16_Q3_LEVEL_COAST_HOLD_PA10_RCT6"
...
#define Q3_FINAL_TOLERANCE_MM            4L
#define Q3_FINAL_SPEED_MM_S           6.0f
#define Q3_FINAL_STABLE_FRAMES          6U
#define Q3_LATCH_SPEED_MM_S           12.0f
...
#define Q3_LANDING_STICTION_ENTER_MM      4L
#define Q3_LANDING_STICTION_SPEED_MM_S   4.0f
#define Q3_LANDING_STICTION_COUNTS      45.0f
...
#define Q3_REVERSAL_OUTPUT_COUNTS      45.0f
#define Q3_REVERSAL_KD_COUNTS_S_PER_MM  0.45f
#define Q3_REVERSAL_TARGET_SLEW_COUNTS_S 1400.0f
#define Q3_PREDICT_DECEL_MM_S2        360.0f
#define Q3_PREDICT_MIN_SPEED_MM_S      20.0f
#define Q3_PREDICT_FALLBACK_MM          -49L
#define Q3_PREDICT_MAX_DISTANCE_MM      40.0f
#define Q3_PREDICT_FRAME_MARGIN_MM      10.0f
#define Q3_BRAKE_RELEASE_SPEED_MM_S    -12.0f
#define Q3_PREBRAKE_START_MM              0L
#define Q3_COAST_START_MM               -20L
#define Q3_COAST_ENTRY_SPEED_MM_S      -60.0f
#define Q3_HOLD_BIAS_COUNTS            -30.0f
...
static void Q3TaskUpdate(s32 position_mm, u8 new_camera_sample)
{
    ...
    if((s_q3_state == Q3_GO_NEG_50) && (new_camera_sample != 0U))
    {
        ...
        /* The +90-count physical baseline gave about 720 mm/s^2 effective
         * deceleration.  V13 caps the reverse command at 45 counts, so the
         * predictor scales that estimate to 360 mm/s^2.  Inertia,
         * rather than a later return stroke, should complete the travel. */
        if(speed_mm_s >= Q3_PREDICT_MIN_SPEED_MM_S)
        {
            ...
            /* V16 deliberately starts the gentle pre-brake when the ball
             * crosses O.  This leaves enough distance to reduce speed before
             * commanding a level beam at -2 cm for the final coast. */
            if(position_mm <= Q3_PREBRAKE_START_MM)
                should_brake = 1U;
        }
    ...
    if((s_q3_state == Q3_BRAKE_RETURN_NEG) && (new_camera_sample != 0U) &&
       (position_mm <= Q3_COAST_START_MM) &&
       (s_velocity_mm_s >= Q3_COAST_ENTRY_SPEED_MM_S))
    {
        ...
        if((Abs32(position_mm - Q3_NEG_TARGET_MM) <= Q3_FINAL_TOLERANCE_MM) &&
           (AbsFloat(s_velocity_mm_s) <= Q3_LATCH_SPEED_MM_S))
        {
            s_q3_final_stable_frames = Q3_FINAL_STABLE_FRAMES;
            s_q3_state = (s_q3_failure_flags == Q3_FAIL_NONE)
                       ? Q3_HOLD_NEG_DONE : Q3_HOLD_NEG_FAILED;
            s_q3_hold_motor_count = s_encoder.count;
            printf("Q3_LANDING_LATCH,pos_mm=%ld,vel_x10=%ld,elapsed=%lu,flags=%u\r\n",
                   s_q3_brake_return_position_mm,
                   s_q3_brake_return_velocity_x10,
                   s_q3_brake_return_elapsed_ms,
                   (u16)s_q3_failure_flags);
            return;
        }
        s_q3_state = Q3_SETTLE_NEG;
        printf("Q3_LEVEL_COAST,pos_mm=%ld,vel_x10=%ld,elapsed=%lu,target_mm=%ld\r\n",
               s_q3_brake_return_position_mm,
               s_q3_brake_return_velocity_x10,
...
static void RunOuterLoop(void)
{
    ...
    {
        /* +5 cm is only a virtual braking direction.  Make the reverse
         * command proportional to approach speed, so the beam automatically
         * levels as the ball slows instead of holding one large angle until
         * velocity crosses zero. */
        s_q3_drive_assist_active = 0U;
        s_integral_mm_s = 0.0f;
        s_stiction_active = 0U;
        q3_output_limit_counts = Q3_REVERSAL_OUTPUT_COUNTS;
        output = -Q3_REVERSAL_KD_COUNTS_S_PER_MM * s_velocity_mm_s;
        output = ClampFloat(output, 0.0f, q3_output_limit_counts);
        q3_suppress_motion_brake = 1U;
    }
    else if(s_q3_state == Q3_SETTLE_NEG)
    {
        /* From about -2 cm onward the beam is level.  The ball finishes the
         * last 3 cm by inertia; this state only watches for a stable landing. */
        s_q3_drive_assist_active = 0U;
        s_integral_mm_s = 0.0f;
        s_stiction_active = 0U;
        output = 0.0f;
    ...
    {
        /* Counter the measured positive rail bias after the coast has ended.
         * The small feed-forward term moves the equilibrium from about
         * -2 cm back to the requested -5 cm without another large stroke. */
        s_q3_drive_assist_active = 0U;
        s_integral_mm_s = 0.0f;
        s_stiction_active = 0U;
        s_q3_hold_kick_active = 0U;
        q3_output_limit_counts = Q3_HOLD_OUTPUT_LIMIT_COUNTS;
        output = Q3_HOLD_BIAS_COUNTS
               + Q3_HOLD_KP_COUNTS_PER_MM * (float)error_mm
               - Q3_HOLD_KD_COUNTS_S_PER_MM * s_velocity_mm_s;
        output = ClampFloat(output, -q3_output_limit_counts,
                            q3_output_limit_counts);
        q3_suppress_motion_brake = 1U;
    }
    else if(s_q3_state == Q3_HOLD_NEG_FAILED)
    {
        ...
        output = ClampFloat(output, -q3_output_limit_counts,
                            q3_output_limit_counts);
        if((Abs32(error_mm) >= Q3_LANDING_STICTION_ENTER_MM) &&
           (AbsFloat(s_velocity_mm_s) <= Q3_LANDING_STICTION_SPEED_MM_S) &&
           (AbsFloat(output) < Q3_LANDING_STICTION_COUNTS))
            output = (error_mm >= 0L) ? Q3_LANDING_STICTION_COUNTS
                                      : -Q3_LANDING_STICTION_COUNTS;
