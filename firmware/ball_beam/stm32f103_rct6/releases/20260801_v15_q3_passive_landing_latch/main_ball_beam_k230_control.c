...
#define CONTROL_FIRMWARE_VERSION       "BALL_BEAM_F103_V15_Q3_PASSIVE_LANDING_LATCH_PA10_RCT6"
...
static void Q3TaskUpdate(s32 position_mm, u8 new_camera_sample)
{
    ...
    if((s_q3_state == Q3_GO_NEG_50) && (new_camera_sample != 0U))
    {
        ...
        if(speed_mm_s >= Q3_PREDICT_MIN_SPEED_MM_S)
        {
            ...
            /* At the measured 40 Hz vision rate the ball travels about
             * 2.6 mm per frame at 103 mm/s.  Trigger one frame before the
             * mathematical boundary so the next fresh sample cannot make
             * the brake command late by that full distance. */
            if(predicted_stop_mm <= ((float)Q3_NEG_TARGET_MM
                                      + Q3_PREDICT_FRAME_MARGIN_MM))
                should_brake = 1U;
        }
...
static void RunOuterLoop(void)
{
    ...
    {
        /* A valid predicted landing already has near-zero ball speed.  Level
         * the beam and leave the ball undisturbed instead of restarting a
         * position loop that can pull it away from the achieved -5 cm. */
        s_q3_drive_assist_active = 0U;
        s_integral_mm_s = 0.0f;
        s_stiction_active = 0U;
        s_q3_hold_kick_active = 0U;
