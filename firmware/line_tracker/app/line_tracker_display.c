...
static void render(line_tracker_state_t state, uint32_t elapsed_ms)
{
    ...
    column = draw_text(1U, 0U, "TIME ");
    column = draw_u32(1U, column, seconds, 2U);
    column = draw_text(1U, column, ".");
    (void)draw_u32(1U, column, tenths, 1U);
    (void)draw_text(4U, 0U, state_name(state));
}
