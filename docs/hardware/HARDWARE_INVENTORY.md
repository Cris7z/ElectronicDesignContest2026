...
| H-UI-04 | C07A 外接模式按键 ×3 | `FUTURE_CONFIRMED` | 用于后续赛题任务/模式选择；与 BLS 启停键分离。 | 用户确认由 C07A 负责 3 个模式按键；具体器件、网络和功能分配未冻结。 | `PA0/PA1` 已被 OLED 占用，禁止复用；完成 C07A 剩余 IO 审查前不接线、不写代码。见 Q-010。 |
...
| H-VISION-01 | 01Studio CanMV K230 | `FUTURE_CONFIRMED` | 后续球检测与图传。 | 用户于 2026-07-31 确认使用 01Studio。 | 只按该型号对应资料整理；未到计划阶段前不写 K230 代码。 |
...
当前代码只能依赖以下硬件类别：C07A/MSPM0G3507、实际模块化底板、D103A/TB6612、两侧 MG513XP28、CD4051、BLS、LED，以及已经实际占用底板资源的 OLED/调试接口。

以下硬件在当前分支必须保持“有资料、无实现、无正式新接线”：STM32F103RCT6、D36A、MS42CG、42 型步进电机、K230、GC2093、无线图传、电池升级和球杆机构。

## 5. 本阶段不做的事

- 不根据旧文档为 STM32F103RCT6、D36A、MS42CG 或 K230 分配正式 IO；
